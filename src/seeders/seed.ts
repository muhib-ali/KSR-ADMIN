...
async function seed() {
  try {
    ...
    for (const roleTypeData of roleTypesData) {
      const existingRoleType = await roleTypeRepository.findOne({
        where: { name: roleTypeData.name },
      });
      if (!existingRoleType) {
        await roleTypeRepository.save(roleTypeData);
        console.log(`Created role type: ${roleTypeData.name}`);
      } else {
        console.log(`Role type already exists: ${roleTypeData.name}`);
      }
    }

    // Get role types for use in roles
    const roleTypes = await roleTypeRepository.find();
    const systemUsersRoleType = roleTypes.find((rt) => rt.name === "systemUsers");
    const usersRoleType = roleTypes.find((rt) => rt.name === "users");

    // 1) ROLES
    console.log("Seeding roles...");
    const rolesData = [
      { title: "Platform Admin", slug: "platformAdmin", is_active: true, role_type_id: systemUsersRoleType?.id },
      { title: "Platform Manager", slug: "platformManager", is_active: true, role_type_id: systemUsersRoleType?.id },
      { title: "Customer", slug: "customer", is_active: true, role_type_id: usersRoleType?.id },
    ];
